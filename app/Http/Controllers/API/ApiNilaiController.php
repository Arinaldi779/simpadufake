...
class ApiNilaiController extends Controller
{
    ...
    public function inputNilaiDosen(Request $request)
    {
        ...
        $kelasMaster = SiapKelasMaster::with('kelasMk')->findOrFail($request->id_kelas_master);
        $id_kelas_mk = $kelasMaster->id_kelas_mk;

        // Ambil jumlah presensi mahasiswa
        $jumlahHadir = SiapPresensiMhs::where('id_kelas_master', $kelasMaster->id_kelas_master)
            ->where('status_presensi_mhs', true)
            ->count();

        // Total pertemuan berdasarkan presensi dosen
        $totalPertemuan = SiapPresensiDosen::where('id_kelas_mk', $id_kelas_mk)->count();
